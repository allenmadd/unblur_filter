...
def filter_photo_3(photo):
  ...
  #  face=np.fromfile(photo, dtype=np.uint8)
    ...
    blurred_f= gaussian_filter1d(face,1, order=1)
...
def filter_photo_2(photo):
    from PIL import Image, ImageTk, ImageFilter
    img=Image.open(photo)

#    img_sharp= img.filter(ImageFilter.SHARPEN)
   # img_sharp1=img_sharp.filter(ImageFilter.SHARPEN)
   # img_sharp=img_sharp1.filter(ImageFilter.SHARPEN)
    #img_sharp1=img_sharp.filter(ImageFilter.SHARPEN)
   # img_sharp=img_sharp1.filter(ImageFilter.SHARPEN)
    img.show()
...
transformed ="C:/Users/madel/OneDrive/Documents/unblur_filter/alpha.png"

filter_photo_2(transformed)